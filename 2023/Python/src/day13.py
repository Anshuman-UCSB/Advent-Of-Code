...
def isHorizFlip(grid, maxX, x, part2 = False):
	flag = False
	for p in grid:
		newP = P(x+(x+1-p.real), p.imag)
		if 0 <= newP.real <= maxX and newP not in grid:
			if not part2 or (part2 and flag):
				return False
			flag = True
	return not part2 or flag

def isVertFlip(grid, maxY, y, part2 = False):
	flag = False
	for p in grid:
...
def solve(grid, maxX, maxY, part2 = False):
	for x in range(maxX): # horizontal flip across x:x+1
		if isHorizFlip(grid, maxX, x, part2):
			return x+1
	for y in range(maxY): # vertical flip across x:x+1

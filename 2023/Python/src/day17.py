...
class State:
	...
	def __repr__(self):
		return str((self.loss, self.p, self.d, self.c))
	...
	def solve(part1):
		...
		while Q:
			...
			if ((part1 and c < 3) or (not part1 and c<10)) and p+d in grid:
				heapq.heappush(Q, State(loss+grid[p+d], p+d, d, c+1)) # go straight
			d *= 1j
			if (part1 or 4<=c) and p+d in grid:
				heapq.heappush(Q, State(loss+grid[p+d], p+d, d, 1)) # turn clockwise
			d *= -1
			if (part1 or 4<=c) and p+d in grid:
				heapq.heappush(Q, State(loss+grid[p+d], p+d, d, 1)) # turn counterclockwise
